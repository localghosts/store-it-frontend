...
function SellerSignUp() {
  ...
  const [setSendOtpErr] = useState('');
  const [setOtpMsg] = useState('');
  const [setRoles] = useState();
  const [storeName, setStoreName] = useState('');
  const [setSignUpErr] = useState();
  ...
  function signUp() {
    ...
    if (otpVerifySuccess) {
      axios.post('www.google.com', {
        username, email, pass, confirmPass, storeName, storeLogo, storeBanner,
      })
        .then((response) => {
          setSignUpSuccess(response?.data?.success);
          setRoles(response?.data?.roles);
        })
        .catch((err) => { setSignUpErr(err); alert(err); });
    } else {
    ...
    <div>
      <CardContent>
        <div className="loginForm">
          ...
          <div className="formGroup">
            <Button variant="contained" label="Password" sx={{ width: 250, height: 40 }} size="large" onClick={() => sendOtp()}>Send OTP</Button>
          </div>
          <div className="formGroup">
            <TextField required id="outlined-required" label="OTP" sx={{ width: 250, height: 40 }} onChange={(event) => setOtp(event.target.value)} />
          </div>
          <div className="formGroup">
            <Button variant="contained" label="Password" sx={{ width: 250, height: 40 }} size="large" onClick={() => verifyOtp()}>Verify OTP</Button>
          </div>
          <div className="formGroup">
          ...
          <div className="formGroup">
            <Button variant="contained" label="Password" sx={{ width: 250, height: 40 }} size="large" onClick={() => signUp()}>Sign Up</Button>
          </div>
        </div>
      </CardContent>
    </div>
