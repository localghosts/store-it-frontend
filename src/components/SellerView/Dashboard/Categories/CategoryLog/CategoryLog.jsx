...
export default function CategoryLog({categories, setCategories}) {
  ...
  const handleStatus = (id) => {
    setCategories([...categories].map((category, index)=>{
      if (id===index) {return {...category, status:!category["status"]}} else return category;
    }
    ))
  }
    ...
    <div className='categoryLog'>
      <Stack spacing={2}>
        ...
        {categories.map((category, index) => (
            <Card sx={{ display: 'flex', backgroundColor: '#D3D3D3', minHeight: 200, maxWidth: 500, borderRadius: 5 }}>
