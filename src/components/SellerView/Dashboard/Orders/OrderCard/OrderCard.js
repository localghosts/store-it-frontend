...
export default function OrderCard({ history, isAccepted }) {
  ...
  const NotAccepted = () => {
    return <div className = "buttons">
      <Button variant={"contained"} onClick={() => { }}> Accept </Button>
      <Button variant={"contained"} onClick={() => { }}> Reject </Button>
    </div>
  }
  return (
    <div  className="card">
    <Card sx={{ maxWidth: 400, borderRadius: 3, backgroundColor: grey[200]}}>
      <CardHeader
        ...
        title={<span className='orderTitle'>
          <b><h2>Order # {history["orderNumber"]}</h2></b>
          <span> </span>
        </span>}
        subheader={<span>
      ...
      <CardContent className='bordertop'>
        {isAccepted === true ? <AfterAccepted /> : <NotAccepted />}
      </CardContent>
    </Card>
    </div>
  );
